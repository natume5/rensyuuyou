...
"""
それぞれ、該当するインデックス番号が返されていますね。
なお、def 文のブロック内で定義している 
XS, S, M, L, XL 以外を引数に渡すとエラーになります。
これを sort メソッドや sorted 関数のキーとして使うと、
指定の順番でソートすることができます。
以下をご覧ください。
"""
...
print(data_sorted)
# ['XS', 'S', 'S', 'M', 'L', 'L', 'L', 'XL', 'XL']
# ['XS', 'S', 'S', 'M', 'L', 'L', 'L', 'XL', 'XL']

"""
このように size 関数の def ブロックの、 sizeList に書いた順番でソートされます。
このように、def 文によって、比較関数を作ることもできるので、覚えておきましょう。
なお、通常、このような比較関数を作る時は、lambda式を使う方がより良い書き方です。
「Pythonのlambda（ラムダ）式の書き方と使い方まとめ」もあわせてご確認ください。
"""
...
"""
いかがだったでしょうか？
def構文を使った関数の作成は、Pythonでは非常によく行います。
最後に、ポイントをまとめておきますね。
