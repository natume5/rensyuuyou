...
"""
この配列の各要素に１を加算するコードを書きましょう。
上のコードに追加して、次のように書いてください。
"""
...
"""
このように、配列そのものに１を足すと、そのすべての要素に１が足されます。
...
配列同士の四則演算は配列の同じ位置にある要素同士で計算されます。
この場合でも引き算、掛け算、割り算について同じように計算できます。


配列同士の計算方法｜行が足りない場合

配列同士の計算ですが、それぞれの配列のサイズが異なる場合はどのように処理されるのでしょうか。
結論から言うと、サイズの違う配列同士の計算では足りない要素が自動的に補完されます。
次のコードを見てください。
"""
...
"""
この例では２次元配列と１次元配列の和を取っていますが、問題なく計算できています。
結果を見れば分かるように、１次元配列「［１０，２０］」が自動的に
２次元配列「［１０，２０］，［１０，２０］」に変換されて計算がされているのです。
...
"""
この場合、２次元配列ｘ２「［［１０］，［２０］］」が自動的に
「［［１０，１０，１０］，［２０，２０，２０］］」
に変換されていることがわかるかと思います。
ブロードキャストが行われる条件として、
演算対象になる配列同士の列または行のどちらかが同じであることが条件になります。
"""
...
"""
Pythonの外部ライブラリであるNumPyは機械学習や深層学習を行うにあたって
必要不可欠なものになっています。
