...
ここではsqlite3を使用しますが、コネクションオブジェクトを変えれば他のものも使用可能です。
...
以下のサンプルは、articleテーブルをメモリ上のsqlite3に作成し、レコードを2件挿入後、
select文の結果をDataFrameに格納しています。
...
cur.execute('insert into articles values (1, "sample1", "BBBB", "2017-07-15 00:00:00")')
...
df2 = pd.DataFrame([['sample4', 'CCC', '2017-07-16 00:00:00']], 
...
if_existsはデータが既に存在している場合の挙動を設定し、appendかrepleceを選ぶことができます。
