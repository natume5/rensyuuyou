...
print(data1)    # {'key1': 100, 'key2': obj}
print(data2)    # {'key1': 100, 'key2': hoge}

"""
Sample型のオブジェクトを参照する辞書をコピーしていますが、
Sample型のオブジェクトもコピーされ、
data1とdata2それぞれが独立したデータを持っていることが確認できます。
ですが、__deepcopy__メソッドを実装しない場合は、（コード自体は動きますが）
23行目と24行目の出力が同じになります。
