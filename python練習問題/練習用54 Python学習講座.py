...
"""
前回、テーブル間のリレーションを考慮しないJOINについて学習しました。
今回はテーブルクラスにリレーションを定義する方法とそれらが考慮されたJOINについて学習しましょう。

relationshipとjoin
あるテーブルのレコードに対して、別のテーブルのレコードが複数紐づくようなリレーションです。
例えば、ブログシステムにおけるユーザーと投稿の関係などがこれに該当しますね。
子テーブルに外部キーを定義し、親テーブルクラス内で以下のようにrelationship関数を使用します。
...
また、relationship関数の引数にback_populates、backref、uselistなどの引数を
指定することにより、関係の双方向性やユニーク性を定義することができます。
これらの組み合わせによりOne To Many、Many To One、One To One、Many To Many、
